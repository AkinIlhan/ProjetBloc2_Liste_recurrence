...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('controller/ajax.php?get_sites=1')
    ...
    siteSelect.addEventListener('change', () => {
        ...
        if (siteId) {
            fetch(`controller/ajax.php?site_id=${siteId}`)
                .then(response => response.json())
                .then(data => {
    ...
    batimentSelect.addEventListener('change', () => {
        ...
        if (batimentId) {
            fetch(`controller/ajax.php?batiment_id=${batimentId}`)
                .then(response => response.json())
                .then(data => {
                    data.forEach(lieu => {
                        const option = document.createElement('option');
...
function showPopup(message, isError = false) {
    ...
    okBtn.addEventListener('click', function () {
        if (isError) {
            ...
            window.location.href = "index.php"; // Redirige vers la liste après succès
        }
...
document.querySelectorAll('.supp-btn_recurr').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        const onOkClick = () => {
            fetch('controller/ajax-suppression.php', {
                method: 'POST',
