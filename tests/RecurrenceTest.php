...
require_once dirname(__FILE__) . '/../modele/demande.php';
require_once dirname(__FILE__) . '/../modele/ModeleDB.php';
require_once dirname(__FILE__) . '/../securite/session.php';
class RecurrenceTest  extends TestCase{
    ...
    protected function setUp(): void {
        ...
        $db = new Database(true); // true = base de test
    ...
    public function testUserBonId(){
        // Démarre une session
        if(session_status() === PHP_SESSION_NONE){
            session_start();
        }
        // Définit un id utilisateur fictif pour ce test
        login("Admin"); // username présent dans la DB

        $this->assertEquals($_SESSION['id_role'] , 1); // Vérification du role
    }

    public function testUserMavaisId(){
        // Démarre une session
        if(session_status() === PHP_SESSION_NONE){
            session_start();
        }
        ...
        $this->assertNotEquals($_SESSION['id_role'] , 2); // Vérification du role
    }
